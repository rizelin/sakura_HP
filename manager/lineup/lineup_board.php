...
$sql = "SELECT `id` FROM `lineup` WHERE `status` IN (1,2) ORDER BY `registration_datetime` DESC";
...
$sql = "SELECT `id`,`name`,`caption`,`img`
        FROM `lineup` WHERE `status` IN(1,2) ORDER BY `registration_datetime` DESC
...
if ($res = $_link->query($sql)) {
    if ($res->num_rows != 0) {
        ...
        for ($i=0; $i <$count ; $i++) {
          ...
          $lineupList[$i]['caption'] = implode("",$lineupList[$i]['caption'] );
        }
